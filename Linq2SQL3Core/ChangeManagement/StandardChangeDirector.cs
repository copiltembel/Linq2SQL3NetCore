...
namespace System.Data.Linq
{
    ...
    internal class StandardChangeDirector : ChangeDirector
    {
        ...
        internal override int DynamicDelete(IReadOnlyList<TrackedObject> items)
        {
            ...
            if (ret == 0 || ret != items.Count)
            {
                var verifyDeleteResult = GetDeleteVerificationCommand(items);
                return verifyDeleteResult;
            }
            return ret;
        }
        ...
        private int GetDeleteVerificationCommand(IReadOnlyList<TrackedObject> trackedObjects)
        {
            // we don't yet know if the delete failed because the check constraint did not match
            // or item was already deleted.  Verify the item exists
            foreach (var trackedObject in trackedObjects)
            {
                var cmd = GetDeleteVerificationCommand(trackedObject);
                var ret = ((int?)this._context.Provider.Execute(cmd).ReturnValue) ?? -1;
                if (ret == -1)
                {
                    return ret;
                }
