...
#warning [FB] POSSIBLE CLONE OF CODE FOUND IN IProvider.Execute
            ...
            QueryInfo qi = this.BuildQuery(query, annotations);
            CheckSqlCompatibility(qi, annotations);
            ...
            return new AdoCompiledQuery(this, query, qi, factory, subQueries);
        }
        ...
        private ICompiledSubQuery CompileSubQuery(SqlNode query, Type elementType, ReadOnlyCollection<System.Data.Linq.Provider.NodeTypes.SqlParameter> parameters)
        {
            ...
            QueryInfo qi = this.BuildQuery(ResultShape.Sequence, TypeSystem.GetSequenceType(elementType), query, parameters, annotations);
        ...
        /// add annotations for.
        /// </summary>
        private void CheckSqlCompatibility(QueryInfo query, SqlNodeAnnotations annotations)
        {
            if (this.Mode == SqlServerProviderMode.Sql2000 ||
                this.Mode == SqlServerProviderMode.SqlCE)
            {
                CompatibilityCheck.ThrowIfUnsupported(query.Query, annotations, this.Mode);
            }
        }
...
#warning [FB] IMPLEMENT FILE VERSION RETRIEVAL HERE AND REPLACE "1.0"
        ...
        private void AssignParameters(DbCommand cmd, ReadOnlyCollection<SqlParameterInfo> @params, object[] userArguments, object lastResult)
        {
            if (@params != null)
            {
                foreach (SqlParameterInfo pi in @params)
                {
        ...
        /// A result object from which you can obtain the return value and output parameters.
        /// </returns>
        IExecuteResult IProvider.Execute(IReadOnlyList<Expression> queries)
        {
        ...
        /// </summary>
        /// <param name="query">The query.</param>
        /// <param name="queryInfo">The query infos.</param>
        /// <param name="factory">The factory.</param>
        /// <param name="userArguments">The user arguments.</param>
        /// <param name="subQueries">The sub queries.</param>
        /// <returns></returns>
        IExecuteResult IProvider.ExecuteAll(Expression query, QueryInfo queryInfo, IObjectReaderFactory factory, object[] userArguments, ICompiledSubQuery[] subQueries)
        {
            return this.ExecuteAll(query, queryInfo, factory, userArguments, subQueries);
        }
        ...
        private IExecuteResult Execute(Expression query)
        {
            ...
            IExecuteResult result = this.ExecuteAll(query, qi, factory, null, subQueries);
            return result;
        }
        ...
        /// A result object from which you can obtain the return value and output parameters.
        /// </returns>
        private IExecuteResult Execute(IReadOnlyList<Expression> queries)
        {
            this.CheckDispose();
            this.CheckInitialized();
            this.CheckNotDeleted();
            if (queries == null)
            {
                throw Error.ArgumentNull(nameof(queries));
            }
            this.InitializeProviderMode();
            var funcletizedQueries = new List<Expression>(capacity: queries.Count);
            foreach (var query in queries)
            {
                funcletizedQueries.Add(Funcletizer.Funcletize(query));
            }
            ...
            SqlNodeAnnotations annotations = new SqlNodeAnnotations();
            var qises = this.BuildQuery(funcletizedQueries, annotations);
            //CheckSqlCompatibility(qi, annotations);

            for (int i = 0; i < funcletizedQueries.Count; i++)
            {
                LambdaExpression lambda = funcletizedQueries[i] as LambdaExpression;
                if (lambda != null)
                {
                    funcletizedQueries[i] = lambda.Body;
                }
            }

            IExecuteResult result = this.ExecuteAll(funcletizedQueries, new ReadOnlyCollection<QueryInfo>(qises.ToList()), factory: null, userArguments: null, subQueries: null);
            return result;
        }
        ...
        /// <param name="queryInfos">The query information.</param>
        /// <param name="factory">The factory.</param>
        /// <param name="parentArgs">The parent arguments.</param>
        /// <param name="userArgs">The user arguments.</param>
        /// <param name="subQueries">The sub queries.</param>
        ...
        [SuppressMessage("Microsoft.Maintainability", "CA1506:AvoidExcessiveClassCoupling", Justification = "Unknown reason.")]
        [MethodImpl(MethodImplOptions.NoInlining | MethodImplOptions.NoOptimization)]
        private IExecuteResult Execute(IReadOnlyList<Expression> queries, ReadOnlyCollection<QueryInfo> queryInfos, IObjectReaderFactory factory, object[] parentArgs, object[] userArgs, ICompiledSubQuery[] subQueries, object lastResult)
        {
            this.InitializeProviderMode();
        ...
        private string GetCommandTexts(IReadOnlyList<QueryInfo> queryInfos)
        {
            var result = new StringBuilder();
            foreach (var queryInfo in queryInfos)
            {
                result.AppendLine(queryInfo.CommandText);
            }
            return result.ToString();
        }

        private IExecuteResult ExecuteAll(Expression query, QueryInfo queryInfo, IObjectReaderFactory factory, object[] userArguments, ICompiledSubQuery[] subQueries)
        {
            IExecuteResult result = null;
            object lastResult = null;
            result = this.Execute(query, queryInfo, factory, null, userArguments, subQueries, lastResult);
            if (queryInfo.ResultShape == ResultShape.Return)
            {
        ...
        private IExecuteResult ExecuteAll(IReadOnlyList<Expression> queries, ReadOnlyCollection<QueryInfo> queryInfos, IObjectReaderFactory factory, object[] userArguments, ICompiledSubQuery[] subQueries)
        {
            IExecuteResult result = null;
            object lastResult = null;
            result = this.Execute(queries, queryInfos, null, null, userArguments, subQueries, lastResult);
            return result;
        }
        ...
        internal IReadOnlyList<QueryInfo> BuildQuery(List<Expression> queries, SqlNodeAnnotations annotations)
        {
            this.CheckDispose();
            ...
            {
                queries[i] = Funcletizer.Funcletize(queries[i]);
            }
            ...
            var nodes = new List<SqlNode>(capacity: queries.Count);
            foreach (var query in queries)
            {
                nodes.Add(converter.ConvertOuter(query));
            }
            return this.BuildQuery(this.GetResultShape(queries.First()), this.GetResultType(queries.First()), nodes, null, annotations);
        }
        ...
        {
            node = BuildQuery(resultType, node, parentParameters, annotations);

            var parameterizer = new SqlParameterizer(_typeProvider, annotations);
            ...
            return GetQueryInfo(resultShape, resultType, node, parameterizer, formatter);
        }

        [SuppressMessage("Microsoft.Maintainability", "CA1506:AvoidExcessiveClassCoupling", Justification = "These issues are related to our use of if-then and case statements for node types, which adds to the complexity count however when reviewed they are easy to navigate and understand.")]
        private IReadOnlyList<QueryInfo> BuildQuery(ResultShape resultShape, Type resultType, List<SqlNode> nodes, ReadOnlyCollection<System.Data.Linq.Provider.NodeTypes.SqlParameter> parentParameters, SqlNodeAnnotations annotations)
        {
            var result = new List<QueryInfo>(capacity: nodes.Count);
            var parameterizer = new SqlParameterizer(_typeProvider, annotations);
            var formatter = new SqlFormatter();
            if (_mode == SqlServerProviderMode.SqlCE ||
                _mode == SqlServerProviderMode.Sql2005 ||
            ...
            {
                nodes[i] = BuildQuery(resultType, nodes[i], parentParameters, annotations);
                result.Add(GetQueryInfo(resultShape, resultType, nodes[i], parameterizer, formatter));
            }
            return result;
        }

        private SqlNode BuildQuery(Type resultType, SqlNode node, ReadOnlyCollection<Provider.NodeTypes.SqlParameter> parentParameters, SqlNodeAnnotations annotations)
        {
        ...
        private QueryInfo GetQueryInfo(ResultShape resultShape, Type resultType, SqlNode node, SqlParameterizer parameterizer, SqlFormatter formatter)
        {
            ...
            else
            {
                ...
                return new QueryInfo(node, commandText, parameters, resultShape, resultType);
            }
        }
        ...
        private IObjectReaderFactory GetDefaultFactory(MetaType rowType)
        {
            ...
            return this.GetReaderFactory(qis.Query, rowType.Type);
        }
